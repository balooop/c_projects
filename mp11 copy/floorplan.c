/* INTRO PARAGRAPH:
    init_slicing_tree: we implemented the algorithm explained in lab by first checking the base case, that we were at the final node to be created. This 
node became the left child of the parent (because the tree is left skewed) and we assigned it the appropriate data values, using the module array given. If it was not the final node to be created, we created the internal node (cutline) and its right child, which was a module. Then, we use a recursive call to create the left child of cutline.
    postfix_traversal: We implemented the algorithm given in lab by first recursivelly traversing the left subtree and the right subtree. Then, we handled the root element by determinging if it was a cutline or a module. We appropriately assigned it either a cutline or a module. Finally, we increment *nth
    get_total_resource: We used a recursive call to check if the node was a leaf node, if it was, there are no further resources below it, so we return that module's resource value. Otherwise, it is an internal node, so we calculate the resources of that internal node's left and right subtrees recursively and sum them.
    is_internal_node: If the node was an internal node, it has left and right children, so in that case we return 1 and otherwise return 0.
    is_leaf_node: Leaf nodes have no children, so if the passed node had no children, we return 1. Else we return 0.
    is_in_subtree: We check the base case that node b = node a. If not, we recursively call is_in_subtree on the left and right children of that subtree. Otherwise, we return 0.
     rotate: We swap the height and witdth values of the node passed
     recut: We flip the cutlines of the node passed
     swap_module: We swap the module pointers of a and b using a temp variable
     swap_topology: We check where b and a are in relation to eachother in the tree (either to the left or right of the parent), and appropriately switch teh pointers of the parents.

     newly initiated members of 0Block, aka.     
     partners: pbaloo2, edhar2, dilanp2
...
int is_leaf_node(node_t* ptr) {
    if(ptr->right == NULL && ptr->left == NULL) //if the node has no children, return 1
    {
        return 1;
    }
    
    // TODO: (remember to modify the return value appropriately)
    return 0; //else return 0 since if it has children, it isn't a leaf node
}
...
int is_internal_node(node_t* ptr) {
    // TODO: (remember to modify the return value appropriately)
    if (ptr->right != NULL || ptr->left!=NULL) //if the node has any children, return 1 
    {
        return 1;
    }
    return 0; //else return 0 since if a node has no children, it is not an internal node
}
...
int is_in_subtree(node_t* a, node_t* b) {
    // TODO: (remember to modify the return value appropriately)
    if (a == b)                                                // if a==b then a is the same node as b so return 1
    {
        return 1;
    }
    else if (!is_leaf_node(a))                // if the values aren't equal and the node isn't a leaf node, we can continue searching recursively
    {
        return (is_in_subtree(a->right,b)) + (is_in_subtree(a->left, b));    // if 'b' is in the subtree of a's right or left child, it's a descendant of a and thus in its subtree (thanks Math213!)
    }
...
void rotate(node_t* ptr) {
    // TODO:
    int tempHeight = ptr->module->h; //put ptr's height in a temp var
    ptr->module->h = ptr->module->w; //put ptr's width into the height variable of ptr
    ptr->module->w = tempHeight;     //put the height value stored in temp into the width variable
}
...
void recut(node_t* ptr) {
    ...
    // TODO:
    if (ptr->cutline == V) //if the ptr's cutline is a V we change it to an H
    {
        ptr->cutline = H;
    }
    else if (ptr->cutline == H) //if the ptr's cutline is an H we change it to a V
    {
        ptr->cutline = V;
    }
    return;
...
void swap_module(node_t* a, node_t* b) {
    ...
    module_t* tempMod = b->module;                    // stores temp value in a temp var
    b->module = a->module;                                    // makes b's module point to a's module
    a->module = tempMod;                                        // a's module points to b's old module (temp)
...
void swap_topology(node_t* a, node_t* b) {
    ...
    node_t* tempB = b; //makes a temp pointer to store b into
    node_t* aParent = a->parent; //makes a parent pointer for a's parent
    node_t* bParent = b->parent; //makes a parent pointer for b's parent
    
    //checks which side of the parent B is on
    if(bParent->left == b) //checks to see if b is on the left side of its parent
    {
        bParent->left = a; //sets a to be pointed to by b's parent
        a->parent = bParent; //sets a's parent to be b's parent
    }
    else if (bParent->right == b) //else we check if b is on the right of its parent
    {
        bParent->right = a; //sets a to be pointed to by b's parent
        a->parent = bParent; 
    }
    
    if(aParent->left == a) //checks to see if a is on the left side of its parent
    {
        aParent->left = tempB; //sets b to be pointed to by a's parent
        tempB->parent = aParent; //sets b's parent to be a's parent
    }
    else if(aParent->right == a) //else we check if a is on the right of its parent
    {
        aParent->right = tempB; //sets b to be pointed to by a's parent
        tempB->parent = aParent; //sets b's parent to be a's parent
    }
}
...
void postfix_traversal(node_t* ptr, int* nth, expression_unit_t* expression) {
    ...
    // TODO:
    postfix_traversal(ptr->left, nth, expression);                 // right recursive case
    postfix_traversal(ptr->right, nth, expression);                // left recursive case
    if(ptr->cutline != UNDEFINED_CUTLINE)                          //if current node is a cutline
    {
        (expression+(*nth))->cutline = ptr->cutline;               // if this bad boy has a cutline, it got a cutline
        (expression+(*nth))->module = NULL;                        // if this bad boy has a cutline, it got no modules
    }
    if(ptr->module)
    {
        (expression+(*nth))->cutline = UNDEFINED_CUTLINE;    // if this bad boy has a module, it got no cutline
        (expression+(*nth))->module = ptr->module;             // if this bad boy has a module, it got a module
        
    }
    (*nth) = (*nth)+1;                                         // move along, nothing to see here
}
...
{
    // TODO:
    if(is_leaf_node(ptr)) //base: ptr points to a leaf node
    {
        return    ptr->module->resource; //return resource value at current node
    }
    else //recursive: if internal node (cutline or module)
    {
        return get_total_resource(ptr->left) + get_total_resource(ptr->right); // go to left and right nodes of internal node
    }
...
node_t* init_slicing_tree(node_t* par, int n) {
    ...
    // TODO:
    node_t *newNode = (node_t*)malloc(sizeof(node_t));         // creates a new node pointer 
    
    if (n == (num_modules -1))                                 // if we're at the end of the tree, to create the final node
    {
        // assigns data to module pointed to by newNode
        newNode->parent = par;                                 // its parent is the parent passed to the function
// if it'a leaf, it has no left or right children (maybe it should try IVF)
        newNode->left = NULL;                                  
        newNode->right = NULL;
        newNode->cutline = UNDEFINED_CUTLINE;                  // it won't have a cutline, b/c all leaves are modules
        newNode->module = &(modules[n]);                           // the module should point to the correct spot in the array
        
        return newNode;    // returns the pointer to the newly created node
    }
    // case where it's an internal node, creating the right node
    newNode->parent = par;    // its parent is par
    newNode->cutline = V;                                      // all intetrnal nodes are cutlines and not modules
    newNode->module = NULL;                                    // the module is at the current spot in the array
    
    newNode->right = (node_t*)(malloc(sizeof(node_t)));        // its right child gets a pointer
    newNode->right->parent = newNode;                          // the right module of a new node is the child of the node we create, so its parent is newNode
    newNode->right->left = NULL;                               // the right of any new node will be a module, so it has no children
    newNode->right->right = NULL;    // it has no children
    newNode->right->cutline = UNDEFINED_CUTLINE;               // mOdULes dOn'T hAVe cUtLiNeS
    newNode->right->module = &(modules[n]);                    // the right child will point to the thing at one less than n because we use postfix notation
    
    newNode->left = init_slicing_tree(newNode, n+1);           // recursive call to create left child 
    return newNode;
